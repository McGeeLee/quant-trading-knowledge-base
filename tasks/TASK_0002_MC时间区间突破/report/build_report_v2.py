...
OUTPUT = TASK_DIR / "output" / "pdf" / "TASK_0002_MC时间区间突破_v2_回测报告.pdf"
CODE_FILE = TASK_DIR / "MultiCharts" / "v2.txt"
REF_DIR = TASK_DIR / "参考"
...
def on_page(canvas, doc):
    ...
    canvas.drawString(18 * mm, height - 10.5 * mm, "TASK_0002 | MultiCharts 时间区间突破 | v2")
...
def build_story():
        ...
        [
            ...
            info_table(
                [
                    ...
                    ["周期", "1 分钟 K 线"],
                    ["策略范围", "日盘基本型；测试日期由 MC 面板控制"],
                    ["报告日期", "2026-08-14"],
                    ["验收状态", "v2 代码已整理；交易列表与绩效概要待实测补充"],
                ],
                [34 * mm, 128 * mm],
            ),
            Spacer(1, 9 * mm),
        ...
        [
            ...
            info_table(
                [
                    ...
                    ["MultiCharts v1 代码", "已保留", "作为含代码内日期条件的历史版本"],
                    ["MultiCharts v2 代码", "已完成", "当前版本完整源代码收入本报告"],
            ...
            Spacer(1, 8 * mm),
            callout(
                "v2 不再包含 TestStartDate / TestEndDate。进行 7 月 8 日或其他区间验收时，直接在 MultiCharts 面板或图表数据范围中设置测试日期。",
    ...
    chunks = [numbered[i : i + 45] for i in range(0, len(numbered), 45)]
    for index, chunk in enumerate(chunks, 1):
        story.extend(
            [
                PageBreak(),
                para(f"四、MultiCharts v2 完整代码 - {index}/{len(chunks)}", "h1"),
    ...
    story.extend(
        [
            ...
            bullet("同一分钟两侧都被触发，或进场后同棒停利、停损时的 K 棒内成交路径设置。"),
    ...
    story.extend(
        [
            ...
            para("历史代码用于比较写法，不覆盖本报告 v2 源代码。", "caption"),
    ...
    story.extend(
        [
            para("八、v2 修订依据", "h1"),
            para("测试日期改由 MultiCharts 面板控制，代码内不再保留 TestStartDate / TestEndDate。", "body"),
            fit_image(REF_DIR / "代码内测试日期条件截图.png", 145 * mm, 35 * mm),
            Spacer(1, 8 * mm),
            para("内置停利停损可能在进场同一分钟触发；1 分钟 OHLC 无法精确还原分钟内部路径。", "body"),
            fit_image(REF_DIR / "内置停利停损函数截图.png", 145 * mm, 35 * mm),
            Spacer(1, 8 * mm),
            callout(
                "同棒进出场要使用更细数据、Bar Magnifier 或目标环境的 K 棒内回测设置复核，不能把 1 分钟粗粒度路径视为逐笔事实。",
                colors.HexColor("#FFF7ED"),
            ),
            PageBreak(),
        ]
    ...
    story.extend(
        [
            ...
            Spacer(1, 10 * mm),
            callout(
                "版本结论：v2 已移除代码内测试日期，并补充同棒路径风险说明；待补齐 MC 交易列表和策略绩效概要后，才能把状态改为完整验收。"
            ),
        ]
    )
...
def build_pdf():
    ...
    doc = BaseDocTemplate(
        ...
        title="TASK_0002 MultiCharts 时间区间突破 v2 回测报告",
        author="量化学习知识库",
